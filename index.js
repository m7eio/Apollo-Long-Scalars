  ...
  constructor(mode = "Long") {
    ...
    const LongParseLiteral = function (ast) {
      if (ast.kind === Kind.INT) {
        ...
        throw new TypeError(
          `Long cannot represent non-integer value: ${ast.value}`
        );
      }
    };
    ...
    const parseLongValue = function (value) {
      if (value === "") {
        throw new TypeError(
          "The value cannot be converted from Long because it is empty string"
        );
      }
    ...
    const serializeLongValue = function (value) {
      console.log("serializeLongValue: " + value);
      if (value === "") {
        throw new TypeError(
          "The value cannot be converted from Long because it is empty string"
        );
    ...
    const safeParseLiteral = function (ast) {
      if (ast.kind === Kind.INT) {
        ...
        if (number <= MAX_INT && number >= MIN_INT) {
          ...
          throw new TypeError(
            "Long number should be in the range from -(2^53) + 1 to 2^53 - 1"
          );
        }
      } else {
        throw new TypeError(
    ...
    const safeValue = function (value) {
      if (typeof value !== "number") {
        throw new TypeError(
          `Long cannot represent non-integer value: ${value}`
        );
      }
